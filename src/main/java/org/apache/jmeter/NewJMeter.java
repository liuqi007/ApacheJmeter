 ...
 */
public class NewJMeter implements JMeterPlugin {
    private static final String JSR223_INIT_FILE = "jsr223.init.file";

    private static final Logger log = LoggerFactory.getLogger(NewJMeter.class);
    ...
    public void start(String[] args, Map<String, String> aArgs) {
        testData = aArgs;
        start(args);
    }
    /**
    ...
    public void start(String[] args) {
        for (String arg : args) {
            log.info("*****NewJmeter.start.args:"+ arg);
        }
        log.info("*****NewJmeter.start.options:"+ options);
        ...
        try {
            log.info("*****NewJmeter.start.initialzeProperties.start");
            initializeProperties(parser); // Also initialises JMeter logging

            try{
                String subString = testData.get("fullJmxFileName").substring(0, testData.get("fullJmxFileName").lastIndexOf("/"));
                log.info("set scriptdir ---->" + subString);
                JMeterUtils.setProperty("scriptdir", subString);
                JMeterUtils.setProperty("jobId", testData.get("jobId"));

                String oriAndNewFileMapStr = testData.get("oriAndNewFileMap");
                String currIp = testData.get("stressIp");
                String jmxHome = testData.get("jmxHome");
                if (StringUtils.isNotBlank(oriAndNewFileMapStr)) {
                    Map<String, String> oriAndNewFileMap = (Map<String, String>) JSON.parse(oriAndNewFileMapStr);
                    for (Map.Entry<String, String> entry : oriAndNewFileMap.entrySet()) {
                        String downFileName = entry.getKey();
                        if(downFileName.contains(currIp)){
                            downFileName = downFileName.replace(currIp, "");
                            String fullParamFileName = jmxHome + File.separator + downFileName;
                            String paramFileDir = fullParamFileName.substring(0, fullParamFileName.lastIndexOf("/"));
                            JMeterUtils.setProperty("paramdir", paramFileDir);
                            log.info("set paramdir ---->" + paramFileDir);
                            break;
                        }
                    }
                }
...
//                    JMeterGuiLauncher.startGui(initialTestFile);
    ...
    private void displayAsciiArt() {
        try (InputStream inputStream = NewJMeter.class.getResourceAsStream("jmeter_as_ascii_art.txt")) {
            if(inputStream != null) {
...
//                    NewDriver.addURL(path);
    ...
    private void startNonGui(String testFile, String logFile, CLOption remoteStart, boolean generateReportDashboard)
        ...
        NewJMeter driver = new NewJMeter();// TODO - why does it create a new instance?
    ...
    public HashTree loadNewTree(File file) throws Exception {
        HashTree tree = SaveService.loadTree(file);

        String oriAndNewFileMapStr = testData.get("oriAndNewFileMap");
        String ip = testData.get("stressIp");
        String jmxHome = testData.get("jmxHome");
        if (StringUtils.isNotBlank(oriAndNewFileMapStr)) {
            replaceParamFilePath(tree, (Map<String, String>) JSON.parse(oriAndNewFileMapStr), ip, jmxHome);
        }

        try{
            HashTree ltree = SaveService.loadTree(new File(NewDriver.getJMeterDir() + "/bin/templates/backlist.jmx"));
            TestPlan ltp = (TestPlan) ltree.getArray()[0];
            HashTree lis = ltree.get(ltp);

            TestElement ae = (TestElement) lis.getArray()[0];
            Arguments arg = (Arguments) ae.getProperty("arguments").getObjectValue();
            arg.addArgument("stress_machine", testData.get("stressIp"));

            arg.removeArgument("application");
            arg.addArgument("application", testData.get("jobId"), "=");

            arg.removeArgument("influxdbUrl");
            String influxdbUrl = testData.get("influxdbUrl") + "=" + testData.get("influx.db");
            arg.addArgument("influxdbUrl", influxdbUrl, "=");

            tree.add(tree.getArray()[0], lis);
    ...
    public static void replaceParamFilePath(HashTree jmxTree, Map<String, String> oriAndNewFileMap, String currIp, String jmxHome) {
        SearchByClass<CSVDataSet> csvDataSetSearchByClass = new SearchByClass<>(CSVDataSet.class);
        jmxTree.traverse(csvDataSetSearchByClass);
        Collection<CSVDataSet> csvDataSets = csvDataSetSearchByClass.getSearchResults();
        //这里不能判断csvDataSet个数是否相等，多个csvDataSet可能公用同一个csv文件


        for (CSVDataSet csvDataSet : csvDataSets) {
            JMeterProperty oriFileNamePro = csvDataSet.getProperty("filename");
            String oriFileName = oriFileNamePro.getStringValue();

            String oriFileNameNew = getOriFileName(oriFileName);

            for (Map.Entry<String, String> entry : oriAndNewFileMap.entrySet()) {
                log.info("====before.oriFileName:{} replace:{}", oriFileNameNew, entry.getValue());

                if(oriFileNameNew.equals(entry.getValue())){
                    String downFileName = entry.getKey();
                    if(downFileName.contains(currIp)){
                        downFileName = downFileName.replace("currIp", "");
                        oriFileNamePro.setObjectValue(jmxHome + File.separator + downFileName);
                        break;
                    }
                }
            }

            log.info("======after.oriFileName:{}", oriFileNamePro.getStringValue());
        }
        JMeter.convertSubTree(jmxTree);
    }

    public static String getOriFileName(String oriFileName) {
        int lastIndex1 = -1;
        if(oriFileName.contains("/")){
            lastIndex1 = oriFileName.lastIndexOf("/") +1;
        }
        ...
        if(lastIndex1>-1 || lastIndex2>-1){
            if(lastIndex1>lastIndex2){
                oriFileName = oriFileName.substring(lastIndex1);
            }else {
                oriFileName = oriFileName.substring(lastIndex2);
            }
        }
    ...
    int uncaughtExceptionHanderTimes = 0;
    ...
    void runNonGui(String testFile, String logFile, boolean remoteStart, String remoteHostsString, boolean generateReportDashboard)
        ...
        Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
            ...
            public void uncaughtException(Thread t, Throwable e) {
                e.printStackTrace();
                log.info("=========uncaughtExceptionHandler========"+e.getMessage());
                String message = "无法处理的异常，异常类型为："+e;
                if(uncaughtExceptionHanderTimes ==0){
                    uncaughtExceptionHanderTimes ++;
                    noticeServerEnd("error", message);
                }
            }
        });
        ...
        try {
            ...
            if (!remoteStart) {
                ...
                tree.add(tree.getArray()[0], testListener);
            ...
            noticeServerEnd("error", e.getMessage());
            log.error("error in nonguidiver", e);
        }
    }

    public int getUncaughtExceptionHanderTimes() {
        return this.getUncaughtExceptionHanderTimes();
    }

    public void setUncaughtExceptionHanderTimes() {
        this.uncaughtExceptionHanderTimes++;
    }

    public static void noticeServerEnd(String status, String msg) {
        NewDriver.setJMeterObjectNull();

        String noticeUrl = NewJMeter.testData.get("stress.notice.ptp.url");

        CloseableHttpClient httpClient = HttpClients.createDefault();

        HttpPost httpPost = new HttpPost(noticeUrl);
        try {
            List<NameValuePair> paramsForm = new ArrayList<>();
            paramsForm.add(new BasicNameValuePair("jobId", testData.get("jobId")));
            paramsForm.add(new BasicNameValuePair("ip", testData.get("stressIp")));
            paramsForm.add(new BasicNameValuePair("statusCode", status));
            paramsForm.add(new BasicNameValuePair("errorMsg", msg));

            UrlEncodedFormEntity entity = new UrlEncodedFormEntity(paramsForm, "UTF-8");
            httpPost.setEntity(entity);

            String resCode = httpClient.execute(httpPost, new ResponseHandler<String>() {
                @Override
    ...
    private static class ListenToTest implements TestStateListener, Remoteable {
        ...
        public void testEnded(String host) {
            ...
            Object jmeter = NewDriver.getJmeterObject();
            if(jmeter!=null) {
                NewJMeter nj = (NewJMeter) jmeter;
                if(nj.getUncaughtExceptionHanderTimes()==0){
                    noticeServerEnd("success", "");
                }
            }else{
                noticeServerEnd("success", "");
            }
        ...
        public void testStarted(String host) {
            startedRemoteEngines.incrementAndGet();
            final long now=System.currentTimeMillis();
    ...
    public static boolean isNonGUI(){
        return "true".equals(System.getProperty(NewJMeter.JMETER_NON_GUI)); //$NON-NLS-1$
    }
    ...
    private static void startUdpDdaemon(final List<JMeterEngine> engines) {
        ...
        if (port > 1000){
            final DatagramSocket socket = getSocket(port, 4445); //只能一个端口
            if (socket != null) {
        ...
        try {
            while(true) {
                ...
                if (address.isLoopbackAddress()){
                    ...
                    String tmp = command;
                    if(command.indexOf("ClassNotFoundException")>-1){
                        command= "ClassNotFoundException";
                    }
                    ...
                    switch(command) {
                        case "ClassNotFoundException" :
                            ...
                            if(jmeter!=null) {
                                NewJMeter nj = (NewJMeter) jmeter;
                                nj.setUncaughtExceptionHanderTimes();;
                            }
                            noticeServerEnd("error", tmp);
...
//        }
    ...
    public static void stopTest() {
        ...
        String cmd = "StopTestNow";
        socket.send(new DatagramPacket("cmd".getBytes(), cmd.length(), InetAddress.getByName("localhost"), 4445));
