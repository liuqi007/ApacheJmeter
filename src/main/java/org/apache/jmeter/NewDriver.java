...
import org.apache.jmeter.websocket.WebSocketServer;
...
public final class NewDriver {
    ...
    private static final NewDynamicClassloader loader;

    /** The directory JMeter is installed in. */
    public static final String JMETER_INSTALLATION_DIRECTORY;
    ...
    private static NewDynamicClassloader createClassLoader(List<URL> jars) {
        return java.security.AccessController.doPrivileged(
                (java.security.PrivilegedAction<NewDynamicClassloader>) () ->
                        new NewDynamicClassloader(jars.toArray(new URL[jars.size()]))
        );
    }
    ...
    private static JMeterController jm = null;
    ...
    public static void main(String[] args)  {
        System.out.println("start jmeter server");
        if(!EXCEPTIONS_IN_INIT.isEmpty()) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        new WebSocketServer().startSever();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            System.out.println("start jmeter server");
            jm = new JMeterController(loader);

            EventLoopGroup group = new NioEventLoopGroup();
            try {
                ServerBootstrap b = new ServerBootstrap();
                b.group(group).channel(NioServerSocketChannel.class)
                        .localAddress(new InetSocketAddress(15625))
                        .childHandler(new ChannelInitializer<SocketChannel>() {
                            @Override
                            protected void initChannel(SocketChannel socketChannel) throws Exception {
                                socketChannel.pipeline().addLast(new ProtocolDecoder(MAX_FRAME_LENGTH, LENGTH_FIELD_OFFSET, LENGTH_FIELD_LENGTH, LENGTH_ADJUSTMENT, INITIAL_BYTES_TO_STRIP, false));
                                socketChannel.pipeline().addLast(jm);
                            }
                        });
            ...
            System.out.println("start jmeter server ....end ..");
        }
    }
     ...
     */
    public static void setLoggingProperties(String[] args) {
    ...
    public static Object getJmeterObject() {
        return JMeterController.jmeter;
    }

    public static void setJMeterObjectNull() {
        JMeterController.jmeter = null;
    }
}
